...
class location_iterator
{
   protected:
    struct dummy {
        unsigned value;
        dummy(unsigned u) : value(u){};
    };
    size_t pos1d;

    location_iterator(dummy pos) : pos1d{pos.value} {}
...
class location_iterator<X, XX...> : public location_iterator<XX...>
{
   ...
   protected:
    ...
    location_iterator(typename location_iterator<XX...>::dummy pos, unsigned c, CC... cc)
        : location_iterator<XX...>(typename location_iterator<XX...>::dummy(pos.value * X + c), cc...), coordinate(c)
    {
    }
   ...
   public:
    location_iterator() : location_iterator<XX...>(), coordinate{0} {}

    template <typename... CC>
    location_iterator(unsigned c, CC... cc)
        : location_iterator<XX...>(typename location_iterator<XX...>::dummy(c), cc...), coordinate(c)
    {
    }
...
template <bool wrap, unsigned X, unsigned... XX>
class space<wrap, X, XX...> : public space<wrap, XX...>
{
   protected:
    ...
    {
        assert(x < X);
        return x * S + space<wrap, XX...>::template id_helper<X * S>(uu...);
    }

   public:
    static inline constexpr std::string info()
    {
        return std::to_string(X) + (sizeof...(XX) > 0 ? 'x' : ' ') + space<wrap, XX...>::info();
    }
    static inline constexpr unsigned dimension() { return 1 + sizeof...(XX); }
    static inline constexpr unsigned dimension(unsigned D) { return D == 0 ? X : space<wrap, XX...>::dimension(D - 1); }
    static inline constexpr size_t size() { return X * space<wrap, XX...>::size(); }
...
template <bool wrap, unsigned... XX>
class spacehood_offsets : public space<wrap, XX...>
{
   ...
   public:
    ...
    static inline const std::vector<int>& neighbors_offsets(unsigned type_id)
    {
        assert(type_id < all_offsets.size());
        return all_offsets[type_id];
    }
};

template <bool wrap, unsigned... XX>
const std::vector<std::vector<int>> spacehood_offsets<wrap, XX...>::all_offsets = make_neighborhoods<wrap, XX...>();

template <unsigned... XX>
using wrapped_space_offsets = spacehood_offsets<true, XX...>;

template <unsigned... XX>
using unwrapped_space_offsets = spacehood_offsets<false, XX...>;
...
class any_space
{
    typedef spacehood_offsets<wrap, XX...> space;
   ...
   public:
    struct space_iterator {
        ...
        space_iterator(std::vector<T>& d, const location_iterator<XX...>& loc) : _data(d), _loc(loc) {}
        ...
        inline space_iterator& operator++()
        {
            ++_loc;
            return *this;
        }
        inline bool operator!=(const space_iterator& other) { return _loc != other._loc; }
        inline typename std::vector<T>::reference operator[](int offset) { return _data[_loc + offset]; }
    };
    struct const_space_iterator {
        const std::vector<T>& _data;
        location_iterator<XX...> _loc;

        const_space_iterator(const std::vector<T>& d, const location_iterator<XX...>& loc) : _data(d), _loc(loc) {}
        ...
        inline const_space_iterator& operator++()
        {
            ++_loc;
            return *this;
        }
        inline bool operator!=(const const_space_iterator& other) { return _loc != other._loc; }
        inline const T& operator[](int offset) const { return _data[_loc + offset]; }
    };

   public:
    any_space() : data(space::size()) {}
    any_space(T _default) : data(space::size(), _default) {}
    any_space(T (*f)())
    {
        data.reserve(space::size());
        while(data.size() < space::size())
            data.push_back(f());
    }
    ...
    inline space_iterator begin() { return space_iterator(data, space::begin()); }
    inline space_iterator end() { return space_iterator(data, space::end()); }
    template <typename... CC>
    inline space_iterator iterator(CC... cc)
    {
        return space_iterator(data, location_iterator<XX...>(cc...));
    }

    inline const_space_iterator begin() const { return const_space_iterator(data, space::begin()); }
    inline const_space_iterator end() const { return const_space_iterator(data, space::end()); }

    inline static const std::vector<int>& neighbors_offsets(const location_iterator<XX...>& it)
    {
        return space::neighbors_offsets(it.type());
    }
...
using wrapped_space = any_space<T, true, XX...>;

template <typename T, unsigned... XX>
using unwrapped_space = any_space<T, false, XX...>;
